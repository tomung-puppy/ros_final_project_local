...
-   **방식**: **gRPC (HTTP/2)**
-   **선정 이유**:
    -   **성능**: HTTP/2 기반으로 더 효율적인 이진 프로토콜 사용.
    -   **강력한 타입**: Protobuf를 통해 메시지 구조가 명확하게 정의되어 오류 감소.
    -   **다국어 지원**: 다양한 언어로 클라이언트/서버 코드 생성이 용이.
    -   **양방향 스트리밍**: 향후 실시간 추론 스트리밍 등 확장에 유리.
-   **데이터 흐름**:
    -   메인 서버가 AI 서버에 추론할 `image_id`를 gRPC 요청으로 전송합니다.
    -   AI 서버는 `image_id`를 사용하여 이미지를 검색하고 추론을 수행한 후, 결과를 Protobuf 메시지 형식으로 응답합니다.
...
| 메인 서버 <-> AI 서버    | gRPC (HTTP/2)                           | 추론 요청 (Image ID), 추론 결과 (Protobuf) | 성능, 타입 안전성, 확장성 용이                   |
