...
from src.domains.robots.robot import Robot, RobotStatus
from src.domains.robots.robot_repository import IRobotRepository
from src.domains.tasks.task import Task, TaskType
...
class FleetManager:
    ...
    """
    def __init__(self, robot_repo: IRobotRepository, robot_communicator: IRobotCommunicator):
        """
        리포지토리와 커뮤니케이터의 구체적인 구현을 주입받습니다.
        """
    ...
    async def update_robot_status(self, robot_id: int, status: RobotStatus, location: tuple, battery: float) -> Optional[Robot]:
        """
        로봇으로부터 주기적으로 상태를 보고받아 DB를 갱신합니다.
        """
        update_data = {
            "status": status,
            "pose_x": location[0],
